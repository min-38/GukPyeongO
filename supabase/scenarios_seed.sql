...
-- notice
insert into public.scenarios (slug, kind, title, source_label, payload, status, sort_order) values
  ('notice', 'notice', '청년 지원사업 모집 공고 — 낚시 조항', '공지사항', '{"id":"mock-notice","sourceLabel":"공지사항","doc":{"source":"국평오시청 청년정책과","title":"「청년 문해력 향상 지원사업」 참가자 모집 공고","body":["1. 신청기간: 공고일 ~ 금일 18:00까지 (기간 엄수)","2. 모집인원: 0명 (예산 소진으로 이번 회차 신규 선발 없음, 대기 등록만 접수)","3. 대상: 국평오시에 주민등록을 둔 만 19~34세 청년","4. 참가비: 무료 (단, 교재비 50,000원은 참가자 본인 부담)","5. 지원금: 수료 시 최대 300,000원 지급 (출석률·과제 평가에 따라 차등)","6. 오리엔테이션: 신청자 전원 필참 (미참석 시 자동 탈락)"]}}'::jsonb, 'published', 2)
on conflict (slug) do update set
  kind = excluded.kind,
  title = excluded.title,
  source_label = excluded.source_label,
...
select s.id, v.step_key, v.type, v.prompt, v.choices::jsonb, v.answer_index::int,
       v.difficulty::int, v.time_limit_sec::int, v.show_up_to::int, v.extra::jsonb, v.sort_order::int
from (values
  ('chat', 'geumil', '어휘', '뭐라고 답장하지…?', '["네! 금요일까지 자료 만들어 보고드리겠습니다!","네, 오늘 5시에 보고드리겠습니다."]'::jsonb, 1, 2, 20, null, '{"context":[{"speaker":"김부장","text":"오 김대리, 왤케 연락이 안 되나?","at":"오전 9:17"},{"speaker":"김부장","text":"아무튼 금일 보고는 언제 진행할 건가?","at":"오전 9:17"}],"reactCorrect":"그래, 5시에 보지.","reactWrong":"금요일…? 김대리, 금일(今日)이 무슨 뜻인지 아는가?","reactTimeout":"김대리? 읽었으면 답을 해야 할 것 아닌가.","at":"오전 9:18"}'::jsonb, 1),
  ('chat', 'apjon', '문법', '부장님한테 뭐라고 하지…?', '["김 과장님은 외근 나가셨습니다.","김 과장은 외근 나갔습니다."]'::jsonb, 0, 3, 20, null, '{"context":[{"speaker":"김부장","text":"아 그리고, 김 과장은 어디 갔나?","at":"오전 9:19"}],"reactCorrect":"알겠네. 들어오면 나 좀 보자고 하게.","reactWrong":"…자네가 김 과장 상사인가?","reactTimeout":"…자네 지금 내 메시지 씹은 건가?","at":"오전 9:20"}'::jsonb, 2),
  ('chat', 'igil', '어휘', '언제까지 하라는 거지…?', '["네, 오늘 오전까지 수정하겠습니다.","네, 내일 오전까지 수정하겠습니다.","네, 이번 주 금요일까지 수정하겠습니다."]'::jsonb, 1, 1, 20, null, '{"context":[{"speaker":"김부장","text":"그리고 어제 올린 결재, 반려했네.","at":"오전 9:21"},{"speaker":"김부장","text":"익일 오전까지 수정해서 다시 올리게.","at":"오전 9:22"}],"reactCorrect":"그래, 내일 아침에 보지.","reactWrong":"익일(翌日)이 언제인지부터 찾아보게.","reactTimeout":"대답이 없네. 이런 식으로 일할 텐가?","at":"오전 9:23"}'::jsonb, 3)
) as v(slug, step_key, type, prompt, choices, answer_index, difficulty, time_limit_sec, show_up_to, extra, sort_order)
join public.scenarios s on s.slug = v.slug
on conflict (scenario_id, step_key) do update set
  type = excluded.type,
