...
-- notice
insert into public.scenarios (slug, kind, title, source_label, payload, status, sort_order) values
  ('notice', 'doc', '청년 지원사업 모집 공고 — 낚시 조항', '공지사항', '{"id":"mock-notice","sourceLabel":"공지사항","doc":{"source":"국평오시청 청년정책과","title":"「청년 문해력 향상 지원사업」 참가자 모집 공고","body":["1. 신청기간: 공고일 ~ 금일 18:00까지 (기간 엄수)","2. 모집인원: 0명 (예산 소진으로 이번 회차 신규 선발 없음, 대기 등록만 접수)","3. 대상: 국평오시에 주민등록을 둔 만 19~34세 청년","4. 참가비: 무료 (단, 교재비 50,000원은 참가자 본인 부담)","5. 지원금: 수료 시 최대 300,000원 지급 (출석률·과제 평가에 따라 차등)","6. 오리엔테이션: 신청자 전원 필참 (미참석 시 자동 탈락)"]}}'::jsonb, 'published', 2)
on conflict (slug) do update set
  kind = excluded.kind,
  title = excluded.title,
  source_label = excluded.source_label,
