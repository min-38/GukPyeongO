...
-- time_limit_sec는 형식/유형별 권장 시간(#34): 짧은 혼동/시간 12초, 한자/사자성어 15초, 공지문 20초.
insert into public.questions (type, prompt, choices, answer_index, time_limit_sec, sort_order) values
  ('time', '친구가 ''사흘 뒤에 보자''고 했다. 사흘 뒤는 언제인가?', '["3일 뒤","4일 뒤","일주일 뒤","오늘"]', 0, 12, 1),
  ('hanja', '공지에 적힌 ''금일(今日)''은 언제를 뜻하는가?', '["오늘","금요일","이번 주","다음 날"]', 0, 15, 2),
  ('confusable', '''심심(甚深)한 사과 말씀드립니다''에서 ''심심한''의 뜻은?', '["지루한","매우 깊은","간이 싱거운","별것 아닌"]', 1, 12, 3),
  ('notice', '공지: ''모집 인원 0명''. 이 공고의 의미로 알맞은 것은?', '["더 이상 뽑지 않는다","새로 0명을 선발한다","누구나 지원 가능하다","무료로 모집한다"]', 0, 20, 4),
  ('hanja', '''무운(武運)을 빈다''는 무슨 뜻인가?', '["운이 없기를 빈다","싸움에서의 행운을 빈다","아무 일 없기를 빈다","운을 시험한다"]', 1, 15, 5),
  ('time', '''모레''는 오늘로부터 며칠 뒤인가?', '["1일 뒤","2일 뒤","3일 뒤","4일 뒤"]', 1, 12, 6),
  ('confusable', '''물건값을 치러 거래를 끝냄''을 뜻하는 말은?', '["결재","결제","결정","결산"]', 1, 12, 7),
  ('notice', '공지: ''우천 시 행사는 익일로 순연됩니다.'' 익일은 언제인가?', '["당일","다음 날","전날","주말"]', 1, 20, 8),
  ('hanja', '''고지식하다''의 뜻으로 알맞은 것은?', '["아는 것이 많다","성질이 곧아 융통성이 없다","지위가 높다","교양이 풍부하다"]', 1, 15, 9),
  ('confusable', '''사과의 색은 바나나와 ___.'' 빈칸에 알맞은 말은?', '["틀리다","다르다","맞다","같다고 틀리다"]', 1, 12, 10);
