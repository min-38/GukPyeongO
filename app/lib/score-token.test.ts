...
describe("grade token", () => {
  it("발급한 토큰은 같은 시크릿으로 검증되어 등급과 회차를 돌려준다", () => {
    const token = createGradeToken(3, ROUND, SECRET);
    expect(verifyGradeToken(token, SECRET)).toEqual({
      grade: 3,
      roundId: ROUND,
    });
  });

  it("회차 없이 발급한 토큰(v1 경로)은 roundId가 null", () => {
    const token = createGradeToken(3, null, SECRET);
    expect(verifyGradeToken(token, SECRET)).toEqual({
      grade: 3,
      roundId: null,
    });
  ...
  it("페이로드 위변조(등급 조작) 시 검증 실패", () => {
    const token = createGradeToken(9, ROUND, SECRET);
    const [, round, exp, sig] = token.split(".");
    const forged = `1.${round}.${exp}.${sig}`; // 9등급을 1등급으로 바꿔치기
    expect(verifyGradeToken(forged, SECRET)).toBeNull();
  });

  it("회차 바꿔치기 시 검증 실패", () => {
    const token = createGradeToken(3, ROUND, SECRET);
    const [grade, , exp, sig] = token.split(".");
    const forged = `${grade}.bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb.${exp}.${sig}`;
    expect(verifyGradeToken(forged, SECRET)).toBeNull();
  });

  it("만료된 토큰은 검증 실패", () => {
    const past = Date.now() - 2 * 60 * 60 * 1000; // 2시간 전 발급
    const token = createGradeToken(5, ROUND, SECRET, past);
  ...
  it("형식이 깨진 토큰은 null", () => {
    ...
    // 회차 칸이 없던 옛 3조각 토큰
    expect(verifyGradeToken("3.1900000000000.sig", SECRET)).toBeNull();
  });
});
